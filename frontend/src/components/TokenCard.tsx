...
export default function TokenCard({ address }: { address: string }) {
        ...
        return (
            <AnimatePresence>
                <motion.div className="absolute w-full h-full bg-black/65 flex justify-center items-center top-0 left-0 z-50" >
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.15, ease: "easeInOut" }} className="bg-gray-800 relative rounded-lg p-8 w-[5/12] h-[55%] flex items-center flex-wrap">
                        ...
                        <div className="ml-4 flex flex-col items-center">
                            ...
                            {isBuying &&
                                <>
                                    ...
                                    <button type="button" className="bg-blue-500 text-white px-4 py-2 rounded-lg mt-4 cursor-pointer hover:bg-blue-700 transition-colors">Confirm</button>
                                </>
                            }
                            {isSelling &&
                                <>
                                    <input type="text" onChange={(e) => { setSellingAmount(!isNaN(Number(e.target.value)) ? e.target.value : "0") }} className="mb-4 mt-4 bg-gray-500 rounded-2xl p-2 " placeholder="Buying Amount" />
    ...
    return (
        <AnimatePresence>
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ ease: "easeInOut", delay: 0.4 }} onClick={() => setIsSelected(!isSelected)} className="bg-gray-800 flex rounded-lg w-108 p-4 items-center cursor-pointer hover:shadow-lg hover:shadow-500/50 transition-shadow">
                <img
